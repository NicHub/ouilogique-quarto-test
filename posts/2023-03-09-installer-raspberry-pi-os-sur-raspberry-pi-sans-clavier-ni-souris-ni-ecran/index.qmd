...
draft: false
image: ./images/Raspberry_Pi_Logo-thumb.png
lang: fr
layout: page
lightbox: auto
...
<p class="text-center" style="margin: 40px 0;">
    <img src="./images/Raspberry_Pi_Logo-thumb.png" alt="Raspberry Pi — Thumb" class="thumb">
</p>
...
> N.B. L’article original, écrit en août 2017, s’intitulait “Installer Raspbian Stretch sur Raspberry Pi” et était accessible à cette URL :
...
-   Un Raspberry Pi modèle 3 (ou plus) pour Raspberry Pi OS 64 bit, ou un Raspberry Pi modèle 2 (ou plus) pour Raspberry Pi OS 32 bit
...
> N.B. Ce chapitre a été mis à jour en mai 2026
...
_Etcher_ a été remplacé par _Raspberry Pi Imager_ et on n’a plus besoin de télécharger l’OS séparément, car c’est le programme qui s’en charge.
...
-   **Important :** Ouvrir les options de personnalisation de _Raspberry Pi Imager_, puis configurer au minimum le nom d’hôte, le nom d’utilisateur, le mot de passe et l’accès distant SSH.
    ...
    > Cette méthode existe encore, mais il faut alors aussi créer un utilisateur manuellement.
...
-   Après environ 30 secondes, ouvrir un terminal et se connecter au Raspberry avec la commande `ssh <utilisateur>@raspberrypi.local`.
    Il n’y a plus de compte `pi` avec le mot de passe `raspberry` par défaut : il faut utiliser le nom d’utilisateur et le mot de passe configurés dans _Raspberry Pi Imager_.
    ...
    Donc pour aller plus loin, il faut supprimer les lignes qui commencent par `raspberrypi.local` dans le fichier `known_hosts`, par exemple avec `ssh-keygen -R raspberrypi.local`.
...
Sous macOS, la commande `nmap` peut être installée via Homebrew.
...
> Explication de l’ordre de lecture des fichiers de configuration :
> <https://linuxize.com/post/bashrc-vs-bash-profile/>.

Créer un fichier bashrc personnel.

```bash
mkdir -p ~/.config/bash/
nano ~/.config/bash/bashrc
```

Copier-coller les commandes suivantes dans ~/.config/bash/bashrc :
...
Sourcer ce fichier dans ~/.bashrc

```bash
nano ~/.bashrc

USER_BASHRC="$HOME/.config/bash/bashrc"; [ -f $USER_BASHRC ] && source $USER_BASHRC
...
## Générer une locale manquante

Si le shell affiche un avertissement du type suivant au démarrage,
c’est généralement qu’une variable d’environnement demande une locale qui n’a pas encore été générée sur le Raspberry.

```bash
bash: warning: setlocale: LC_ALL: cannot change locale (fr_CH.UTF-8): No such file or directory
```

On peut vérifier les locales disponibles avec la commande suivante :

```bash
locale -a
```

Si `fr_CH.utf8` n’apparaît pas dans la liste, il faut activer la ligne correspondante dans `/etc/locale.gen`, puis régénérer les locales.

```bash
sudo sed -i 's/^# *fr_CH.UTF-8 UTF-8/fr_CH.UTF-8 UTF-8/' /etc/locale.gen
sudo locale-gen fr_CH.UTF-8
```

On peut ensuite vérifier que la locale a bien été générée.

```bash
locale -a | grep '^fr_CH\.utf8$'
```

> N.B. Cette procédure ne change pas forcément la langue par défaut du système.
> Elle rend seulement la locale `fr_CH.UTF-8` disponible.
> Pour changer la langue par défaut, utiliser `sudo raspi-config` ou `sudo update-locale`.
...
# Puis met à jour les paquets installés.
sudo apt update
sudo apt full-upgrade -y

# Si nécessaire supprime les dépendances qui ne sont plus utilisées.
...
```bash
# Si le fichier ~/.ssh/id_ed25519.pub n’existe pas,
# il faut le créer avec `ssh-keygen`.
ssh-keygen -t ed25519 # Accepter toutes les valeurs par défaut.
cat ~/.ssh/id_ed25519.pub # + Copier le résultat dans le presse-papier.
```
...
> Cette clé fonctionne aussi pour se connecter à un Raspberry Pi.
...
Sur les versions récentes de Raspberry Pi OS, le serveur VNC fourni est WayVNC.
Raspberry Pi Connect est aussi une option officielle pour l’accès distant au bureau et au terminal.
...
sudo apt-get purge wolfram-engine # Si le paquet est installé.
...
> N.B. Selon la version de Raspberry Pi OS utilisée, le paquet `wolfram-engine` peut ne pas être disponible.
...
Depuis Raspberry Pi OS Bookworm, `pip` ne doit plus être utilisé pour installer des paquets directement dans le Python du système.
Il faut passer par `apt` ou par un environnement virtuel.
...
```bash
sudo smbpasswd -a <utilisateur> # Pour changer le mot de passe.
```
...
```bash
⌘ K
smb://<utilisateur>@raspberrypi.local
```
...
### Installer d’autres versions de Python 3


```bash
# Vérifier le modèle de RPi
# Par exemple : Raspberry Pi 4 Model B Rev 1.1
cat /proc/device-tree/model

# Vérifier le _machine hardware name_
# Doit être : aarch64
uname -m

# Installer `uv` <https://docs.astral.sh/uv/#installation>
curl -LsSf https://astral.sh/uv/install.sh | sh

. "$HOME/.local/bin/env"

# Lister les versions de Python disponibles avec `uv`
uv python list

# Installer une version au choix
uv python install 3.14

# Créer et activer un environnement virtuel pour cette version de Python.
uv venv ~/.venvs/pyve14 --python 3.14
source ~/.venvs/pyve14/bin/activate

# Ajouter dans bashrc personnel (~/.config/bash/bashrc)
alias pyve14="source ~/.venvs/pyve14/bin/activate"
...
Lorsqu’on connecte un disque externe, il est automatiquement accessible au chemin `/media/<utilisateur>/nom_du_disque`.
...
```bash
ls -l /media/<utilisateur>/
cd /media/<utilisateur>/LaCie
```
...
└─sda2      /dev/sda2       0  0    1 part  4.5T      1T /media/<utilisateur>/LaCie
...
Et ils ont chacun un ou deux points de montage `/media/<utilisateur>/LaCie`, `/boot` et `/`.
...
1. Démonter ses points de montage.
   Dans le cas présenté, il n’en a qu’un qui est monté à `/media/<utilisateur>/LaCie`, mais dont la référence se trouve à `/dev/sda2`.
...
Ceci est valable même si on ne se connecte pas au bureau graphique.
...
- Les stockages MMC (Multi Media Card) sont nommés avec le préfixe `mmcblk`.
 -->
:::
...
-   [Darkone Linux, Quel Linux pour quel usage ?](https://www.youtube.com/watch?v=rlNtyhGLA_Y&t=6s)
